...
async def fetch_feature_flags(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header()):
    feature_flags = await get_feature_flags(db, x_tenant_id)
    return feature_flags

@router.post("/featureflags", response_model=FeatureFlag)
async def post_feature_flag(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), feature_flag_obj: FeatureFlagCreate = Body()):
...
async def modify_feature_flag(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), feature_flag_id: UUID4 = Path(), feature_flag_obj: FeatureFlagPatch = Body()):
...
async def fetch_tag_types(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header()):
    tag_types = await get_tag_types(db, x_tenant_id)
    return tag_types

@router.post("/tagtypes", response_model=TagType)
async def post_tag_type(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), tag_type_obj: TagType = Body()):
    tag_type = await create_tag_type(db, x_tenant_id, tag_type_obj)
    return tag_type

@router.get("/tagvalues", response_model=list[NestedTagValue])
async def fetch_tag_values(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), tag_type_id: UUID4 = Query(default=None)):
    tag_values = await get_tag_values(db, x_tenant_id, tag_type_id)
    return tag_values

@router.post("/tagvalues", response_model=NestedTagValue)
async def post_tag_value(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), tag_value_obj: TagValueCreate = Body()):
    tag_value = await create_tag_value(db, x_tenant_id, tag_value_obj)
    return tag_value

@router.get("/environments", response_model=list[Environment])
async def fetch_environments(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header()):
    environments = await get_environments(db, x_tenant_id)
    return environments

@router.post("/environments", response_model=Environment)
async def post_environment(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), environment_obj: Environment = Body()):
    environment = await create_environment(db, x_tenant_id, environment_obj)
    return environment

@router.get("/idgroups", response_model=list[IdGroup])
async def fetch_id_groups(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header()):
    id_groups = await get_id_groups(db, x_tenant_id)
    return id_groups

@router.post("/idgroups", response_model=IdGroup)
async def post_id_group(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), id_group_obj: IdGroup = Body()):
...
async def change_feature_flag_value(db: AsyncSession = Depends(get_db_session), x_tenant_id: UUID4 = Header(), feature_flag_id: UUID4 = Path(), environment_id: UUID4 = Query(), feature_flag_value_obj: FeatureFlagValueUpdate = Body()):
